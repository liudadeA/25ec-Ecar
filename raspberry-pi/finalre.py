...
# 中心点误差阈值（横纵方向都需小于此值）
CENTER_ERROR_THRESHOLD = 40

# 显示窗口设置
WINDOW_NAME = 'Target Tracker'

# 模式控制变量：0-找中心模式，1-单应性变换模式
operation_mode = 0
...
# ============== 单应性变换相关函数 ==============
def apply_homography(rect, img):
    """
    对检测到的矩形应用单应性变换，将其转换为正视视角
    返回变换后的图像和逆变换矩阵
    """
    # 获取矩形的四个角点
    ...
    # 转换为float32类型，确保符合OpenCV要求
    h = h.astype(np.float32)
        
    # 应用单应性变换
    transformed_img = cv2.warpPerspective(img, h, (width, height))
    
    # 计算逆单应性矩阵（用于将点从正视视角转换回原始视角）
    try:
        h_inv = np.linalg.inv(h)
        h_inv = h_inv.astype(np.float32)  # 确保逆矩阵也是正确类型
    except np.linalg.LinAlgError:
        h_inv = None
    
    return transformed_img, h, h_inv
...
def transform_point(point, h_matrix):
    ...
    # 确保矩阵是正确的类型和形状
    if h_matrix.shape != (3, 3) or (h_matrix.dtype != np.float32 and h_matrix.dtype != np.float64):
        return None
    ...
    # 应用变换
    transformed = h_matrix @ homogeneous
    # 转换回笛卡尔坐标
    if abs(transformed[2]) < 1e-6:  # 避免除以接近零的值
        return None
    return (int(transformed[0] / transformed[2]), int(transformed[1] / transformed[2]))
...
def draw_to_canvas(canvas, show_img, processed_img, tracker, detected_rects, threshold, auto_thresh, fps):
    ...
    # 1. 绘制原始图像到左侧
    canvas[0:PICTURE_HEIGHT, 0:PICTURE_WIDTH] = show_img.copy()
    
    # 2. 绘制处理后的图像到中间（左侧图像右侧）
    if len(processed_img.shape) == 2:
        processed_img = cv2.cvtColor(processed_img, cv2.COLOR_GRAY2BGR)
    canvas[0:PICTURE_HEIGHT, PICTURE_WIDTH:2*PICTURE_WIDTH] = processed_img.copy()
    ...
    # 8. 绘制跟踪状态（在两张图上都显示）
    draw_tracking_status(canvas, tracker, show_img)
    ...
    mode_text = "模式: 找中心" if operation_mode == 0 else "模式: 单应性变换"
    cv2.putText(canvas, mode_text, (SLIDER_X-90, SLIDER_Y + SLIDER_HEIGHT + 55), 
...
def draw_tracking_status(canvas, tracker, original_img):
    """绘制跟踪状态标记（在两张图上都显示）"""
    if not tracker.locked:
        return
    ...
    # 模式1: 单应性变换模式
    elif operation_mode == 1 and tracker.nested_rects:
        # 对第一个矩形应用单应性变换
        rect = tracker.nested_rects[0]
        transformed_img, h_matrix, h_inv = apply_homography(rect, original_img)
        
        if transformed_img is not None and h_inv is not None:
            # 在正视图像中心绘制圆形
            ...
            # 生成圆上的点并绘制在正视图像上
            circle_points = generate_circle_points(center正视, radius, CIRCLE_POINTS)
            for i in range(len(circle_points) - 1):
                cv2.line(transformed_img, 
                         (int(circle_points[i][0]), int(circle_points[i][1])),
                         (int(circle_points[i+1][0]), int(circle_points[i+1][1])),
                         (0, 0, 255), 2)
            
            # 将正视图像缩放到合适大小并显示在中间处理图的角落
            scale = 0.3
            small_h, small_w = int(h * scale), int(w * scale)
            transformed_small = cv2.resize(transformed_img, (small_w, small_h))
            
            # 放置在中间处理图的左上角
            canvas[10:10+small_h, PICTURE_WIDTH+10:PICTURE_WIDTH+10+small_w] = transformed_small
            
            # 将正视图像中的整个圆投影回原始图像坐标系
            projected_points = []
            for point in circle_points:
                original_point = transform_point(point, h_inv)
                if original_point:
                    projected_points.append(original_point)
            ...
            if len(projected_points) >= 2:
                for i in range(len(projected_points) - 1):
                    ...
                    # 在中间处理图上也绘制
                    cv2.line(canvas, 
                             (projected_points[i][0] + PICTURE_WIDTH, projected_points[i][1]),
                             (projected_points[i+1][0] + PICTURE_WIDTH, projected_points[i+1][1]),
                             (0, 0, 255), 2)
            
            # 绘制圆心
            original_center = transform_point(center正视, h_inv)
            if original_center:
                # 在原始图像上绘制转换后的圆心
                cv2.circle(canvas, original_center, 5, (0, 255, 255), -1)
                # 在中间处理图上也绘制
                cv2.circle(canvas, (original_center[0] + PICTURE_WIDTH, original_center[1]), 
                          5, (0, 255, 255), -1)
                
                # 绘制从矩形中心到转换后圆心的连线
                rect_center = tracker.calculate_actual_center(rect)
                cv2.line(canvas, rect_center, original_center, (0, 255, 255), 2)
                cv2.line(canvas, 
                         (rect_center[0] + PICTURE_WIDTH, rect_center[1]), 
                         (original_center[0] + PICTURE_WIDTH, original_center[1]), 
                         (0, 255, 255), 2)
...
def send_uart_data(u1, tracker, fps):
    ...
    if operation_mode == 0:
        ...
        # 模式1: 尝试发送单应性变换后的圆心坐标
        tx, ty = tracker.position[0], tracker.position[1]  # 默认使用原始中心
        if tracker.nested_rects:
            rect = tracker.nested_rects[0]
            # 使用原始图像的一小部分来计算单应性变换，避免创建大的空图像
            h, w = rect.h(), rect.w()
            if h > 0 and w > 0:  # 确保尺寸有效
                # 提取矩形区域的图像用于计算变换
                x, y, _, _ = rect.rect()
                # 确保坐标在有效范围内
                x = max(0, min(x, PICTURE_WIDTH - w))
                y = max(0, min(y, PICTURE_HEIGHT - h))
                rect_img = original_img[y:y+h, x:x+w] if 'original_img' in locals() else None
                
                if rect_img is not None and rect_img.size > 0:
                    _, _, h_inv = apply_homography(rect, rect_img)
                else:
                    _, _, h_inv = apply_homography(rect, np.zeros((h, w, 3), dtype=np.uint8))
    ...
    try:
        ...
        print(f"发送数据: {aaa},{bbb}, FPS: {fps:.1f}, 模式: {operation_mode}")
    except Exception as e:
        print(f"串口发送失败: {e}")
...
# ============== 主程序 ==============
def main():
    global current_threshold, auto_threshold, show_display, operation_mode, original_img
    ...
    print("按 'm' 键切换操作模式 (0-找中心, 1-单应性变换)")
    ...
    try:
        while True:
            ...
            original_img = frame.copy()  # 保存原始图像用于后续处理
            ...
            if key == ord('q'):
                ...
                mode_name = "找中心" if operation_mode == 0 else "单应性变换"
                print(f"操作模式已切换为: {mode_name} (模式{operation_mode})")
