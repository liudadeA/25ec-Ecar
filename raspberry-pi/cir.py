...
def draw_tracking_status(canvas, tracker, original_img):
    ...
    # 模式1: 单应性变换模式
        ...
        if transformed_img is not None and h_inv is not None:
            ...
            center正视 = (w // 2, h // 2)
            radius = min(w, h) // 8
            cv2.circle(transformed_img, center正视, radius, (0, 0, 255), 2)
            ...
            if original_center:
                # 在原始图像上绘制转换后的圆心
                cv2.circle(canvas, original_center, 5, (0, 0, 255), -1)
                # 在中间处理图上也绘制
...
# ============== 主程序 ==============
def main():
    global current_threshold, auto_threshold, show_display, operation_mode
